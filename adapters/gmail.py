...
# DIP-0047: a sent mail is irreversible and was unrecorded. Detail deliberately
# omits recipient and subject — the ledger converges to git, and an attestation
# is not the place to replicate a mailbox.
...
class GmailAdapter:
    ...
    @attests("email.sent",
             ref=lambda r: r or "",
             detail=lambda r: "sent" if r else "send returned no id")
    def send_email(
        self,
        to: List[str],
        subject: str,
    ...
    @attests("email.reply", ref=lambda r: r or "")
    def reply(
        self,
        thread_id: str,
        message_id: str,
